...
function UI:RefreshMenu()
	for i, b in ipairs(self.slotButtons) do
		...
		if used then
			...
			ns.Tooltip(b, G:SlotName(i), format("%s  -  %s", class and ns:Trans(class.name) or "?", format(ns:Trans("LID_LEVEL"), slot.level or 1)), slot.hardcore and format("|cffff4040%s|r", ns:Trans("LID_HARDCORE")) or nil)
		else
...
function UI:CreateClassPage(parent)
	...
	ns.Tooltip(hardcore, format("|cffff4040%s|r", ns:Trans("LID_HARDCORE")), ns:Trans("LID_HARDCORE_HINT"))
