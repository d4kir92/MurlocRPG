...
function Shop:Create()
	...
	for i = 1, COLS * BUY_ROWS do
		...
		local b = ns.ItemSlot(f, SLOT, function(self2)
			Shop:OnBuy(self2.itemId, self2)
		end)
	...
	for i, supply in ipairs(ns.CONSUMABLES) do
		local b = ns.ItemSlot(f, SLOT, function(self2)
			Shop:OnBuySupply(supply.id, self2)
		end)
	...
	for i = 1, COLS * SELL_ROWS do
		...
		local b = ns.ItemSlot(f, SLOT, function(self2)
			Shop:OnSell(self2.bagIndex, self2)
		end)
...
function Shop:Refresh()
	...
	for i, b in ipairs(self.buySlots) do
		...
		if item then
			...
			b.icon:SetVertexColor(G:CanAfford(item.value) and 1 or 0.9, G:CanAfford(item.value) and 1 or 0.4,
				G:CanAfford(item.value) and 1 or 0.4)
