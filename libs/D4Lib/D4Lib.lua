...
if _G["C_Timer"] == nil then
    ...
    C_Timer.After = function(duration, callback) tinsert(f.tab, {GetTime() + duration, callback}) end
    D4.oldWow = true
end
...
eaf.tab = {}
eaf:Hide()
eaf:SetScript("OnUpdate", function(self, elapsed)
    ...
    if #self.tab == 0 then self:Hide() end
end)
...
function D4:UnregisterEvent(frame, event)
    ...
    if C_EventUtils.IsEventValid(event) then frame:UnregisterEvent(event) end
end
...
function D4:OnEvent(frame, callback, from)
    ...
    frame:HookScript("OnEvent", function(sel, event, ...)
        if debug then
...
D4:OnEvent(fSecure, function()
    xpcall(function(calls)
...
function D4:SafeExec(sel, func, from)
    ...
    local ok = xpcall(function(fun) func() end, function(err) end, func)
    if not ok then callbacks[from] = func end
end
...
function D4:GetContainerNumSlots(bagID)
    ...
    local max = cur
    if bagID == 0 and IsAccountSecured and not IsAccountSecured() then max = cur + 4 end
    return max, cur
end
...
local function FixIconChat(sel, event, message, author, ...)
    if ICON_LIST then
        for tag in string.gmatch(message, "%b{}") do
            ...
            if ICON_TAG_LIST_EN[term] and ICON_LIST[ICON_TAG_LIST_EN[term]] then message = string.gsub(message, tag, ICON_LIST[ICON_TAG_LIST_EN[term]] .. "0|t") end
        end
    end
...
D4:After(2, function()
    ...
    for i, v in pairs(_G) do
        if string.find(i, "CHAT_MSG_", 1, true) and not tContains(chatChannels, i) then tinsert(chatChannels, i) end
    end
...
if false then
    for i, v in ipairs(C_Texture.GetAtlasElements()) do
        if v:lower():find("raceicon") and (v:lower():find("dwarf") or v:lower():find("dwarf")) then D4:MSG("Möglicher Key: " .. v) end
    end
end
...
function D4:GetRaceIcon(race, gender)
    ...
    race = race:lower()
    if raceAtlasFix[race] ~= nil and C_Texture.GetAtlasInfo(D4:GetRaceAtlas(race, genderNames[gender])) == nil then race = raceAtlasFix[race] end
...
function D4:GetTalentInfo()
    ...
    elseif D4:GetWoWBuild() ~= "TBC" and GetPrimaryTalentTree and GetPrimaryTalentTree() then
        ...
        if specid and GetTalentTabInfo then _, _, _, icon = GetTalentTabInfo(specid) end
        return specid, icon
    elseif GetTalentTabInfo then
        local ps = 0
        for i = 1, 4 do
...
function D4:GetHeroSpecId()
    ...
    if C_ClassTalents and C_ClassTalents.GetActiveHeroTalentSpec then heroSpecID = C_ClassTalents.GetActiveHeroTalentSpec() end
    return heroSpecID
end
...
D4:OnEvent(f, function(self, event, ...)
...
function D4:DrawDebug(name, callback, fontSize, sw, sh, p1, p2, p3, p4, p5)
    ...
    if fontSize then D4:SetFontSize(fDebug.header, fontSize) end
    ...
    if fontSize then D4:SetFontSize(fDebug.text, fontSize) end
    local function Think()
        local text = callback()
        fDebug.text:SetText(text)
        D4:After(0.2, function() Think() end, "D4:DD " .. name)
    end
...
function D4:EasyFind(word, exact)
    ...
    for i, v in pairs(_G) do
        if exact then
            if i and type(i) == "string" and string.lower(i) == word then D4:MSG("i", i, "v", v) end
            if v and type(v) == "string" and string.lower(v) == word then D4:MSG("i", i, "v", v) end
        else
            if i and type(i) == "string" and string.find(string.lower(i), word, 1, true) then D4:MSG("i", i, "v", v) end
            if v and type(v) == "string" and string.find(string.lower(v), word, 1, true) then D4:MSG("i", i, "v", v) end
        end
    end
end
...
function D4:FindInGlobal(name, exact, ...)
    local args = {...}
    D4:After(0.1, function()
        for i, v in pairs(_G) do
            if exact then
                if v and type(v) == "string" and v == name then D4:MSG("i", i, "v", v) end
            else
                if v and type(v) == "string" and string.find(v, name, 1, true) then
                    if #args > 0 then
                        ...
                        if all then D4:MSG("v", v, "i", i) end
                    else
                        D4:MSG("v", v, "i", i)
                    end
                end
...
D4:After(1, function()
    if (D4:GetWoWBuild() == "TBC" or D4:GetWoWBuild() == "RETAIL") and PlayerFrame.RoleFix == nil then
        ...
        local function SetupRoleMenu(ownerRegion, rootDescription, contextData)
            ...
            local tankBtn = roleMenu:CreateRadio("|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES:16:16:0:0:64:64:0:19:22:41|t " .. D4:Trans("LID_TANK"), function() return IsRole(unit, "TANK") end, function() UnitSetRole(unit, "TANK") end)
            tankBtn:SetEnabled(CanClassBeRole(unit, "TANK"))
            local healBtn = roleMenu:CreateRadio("|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES:16:16:0:0:64:64:20:39:1:20|t " .. D4:Trans("LID_HEALER"), function() return IsRole(unit, "HEALER") end, function() UnitSetRole(unit, "HEALER") end)
            healBtn:SetEnabled(CanClassBeRole(unit, "HEALER"))
            local dpsBtn = roleMenu:CreateRadio("|TInterface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES:16:16:0:0:64:64:20:39:22:41|t " .. D4:Trans("LID_DAMAGER"), function() return IsRole(unit, "DAMAGER") end, function() UnitSetRole(unit, "DAMAGER") end)
            dpsBtn:SetEnabled(CanClassBeRole(unit, "DAMAGER"))
            roleMenu:CreateRadio(D4:Trans("LID_NOROLE"), function() return IsRole(unit, "NONE") end, function() UnitSetRole(unit, "NONE") end)
        end
        ...
        for _, menuType in ipairs(menuTypes) do
            Menu.ModifyMenu(menuType, function(ownerRegion, rootDescription, contextData) SetupRoleMenu(ownerRegion, rootDescription, contextData, "target") end)
        end
    end
end, "SetRole FIX")
    ...
    if MBTNS == nil then
        ...
        if MICRO_BUTTONS == nil then
            ...
            for i, v in pairs(MICRO_BUTTONS) do
                if v ~= "SocialsMicroButton" then tinsert(MBTNS, v) end
            end
        end
        ...
        elseif D4:GetWoWBuild() == "CLASSIC" then
            MBTNS = {"CharacterMicroButton", "SpellbookMicroButton", "TalentMicroButton", "QuestLogMicroButton", "GuildMicroButton", "LFDMicroButton", "WorldMapMicroButton", "MainMenuMicroButton", "HelpMicroButton"}
        end
    end
    return MBTNS
end
